...
error_x = np.abs(data_dopri5["x"] - data_rk4["x"])
error_y = np.abs(data_dopri5["y"] - data_rk4["y"])
...
plt.subplot(1, 1, 1)
plt.plot(time, error_x, label="DOPRI5 vs RK4 (x)", color="yellow")
plt.plot(time, error_y, label="DOPRI5 vs RK4 (y)", color="purple")
plt.xlabel("Time (t)")
